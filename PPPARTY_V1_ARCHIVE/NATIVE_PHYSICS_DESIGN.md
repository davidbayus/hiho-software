...
**No wrist chain in V1** (decision 2026-04-23, David): palm plate attaches directly to forearm tip. Revisit after first tuning pass if the wrist lacks whip.
...
**Total new state items: 48.** Added to existing 11 → 59 total. Re-eval cost: modest (state is just vec3 read/writes; actual sim math already scales with chain count).
...
- **Wrist** — V1 has no dedicated wrist segment. Palm plate attaches directly to the forearm tip (David's gut call, 2026-04-23: simplest starting topology; revisit after first tuning pass).
...
1. ~~**Wrist: 1 or 2 segments?**~~ **RESOLVED 2026-04-23 (David):** zero wrist segments in V1. Palm attaches directly to forearm tip. Simplest starting topology. Revisit in V1.1 if the wrist lacks whip — we can add a single wrist bone, or the two-segment `wrist_A → wrist_B` chain from `HAND_TRACKING_DESIGN.md`, as a swap-in.
