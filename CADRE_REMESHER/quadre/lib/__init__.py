"""
QUADRE — Native library wrapper for QuadWild-BiMDF.

This is the low-level interface to the pre-compiled QuadWild libraries.
Based on QRemeshify by ksami (GPL-3.0), which wraps QuadWild-BiMDF.
"""
...
class QuadreException(Exception):
    pass
...
class Quadwild():
    def __init__(self, mesh_path: str) -> None:
        if mesh_path is None or len(mesh_path) == 0:
            raise QuadreException("mesh_path is empty")
    ...
    def remeshAndField(self, remesh: bool, enableSharp: bool, sharpAngle: float) -> None:
        ...
        try:
            ...
            raise QuadreException("remeshAndField failed") from e
    ...
    def trace(self) -> bool:
        ...
        try:
            ...
            raise QuadreException("trace failed") from e

    def quadrangulate(self, params: QRParameters, scaleFact: float,
                      fixedChartClusters: int, enableSmoothing: bool) -> int:
        mesh_path_c = self.traced_path.encode()
        try:
            return self.quadpatches.quadPatches(
                mesh_path_c, byref(params), scaleFact,
                fixedChartClusters, enableSmoothing
            )
        except Exception as e:
            raise QuadreException("quadrangulate failed") from e
